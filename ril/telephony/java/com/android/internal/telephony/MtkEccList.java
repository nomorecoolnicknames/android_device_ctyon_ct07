...
public class MtkEccList extends PhoneNumberUtils {
    ...
    // System property values are limited to 91 characters. The merged ECC
    // list easily exceeds that, and SystemProperties.set() would then throw
    // while GET_SIM_STATUS is processed, so truncate on a number boundary.
    // isEmergencyNumberExt() still uses the full in-memory list.
