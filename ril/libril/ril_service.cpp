 ...
 * 2017/8/5: patches for vendor version 10		by: daniel_hk
 * 2017/8/5: mark all V13 functions to NOT_SUPPORTED	by: daniel_hk
 * 2017/8/12: add MTK commands and responses		by: daniel_hk
 * 2017/8/12: patch setupDataCall for MTK		by: daniel_hk
 * 2017/8/19: stick with 1.0 for now			by: daniel_hk
 * 2017/8/19: emulate getDeviceIdentity handling	by: daniel_hk
 * 2017/8/26: fix deactivateDataCall			by: daniel_hk
 * 2017/8/26: patch getDataRegistrationStateResponse	by: daniel_hk
 * 2017/8/27: fix getDataRegistrationStateResponse	by: daniel_hk
 * 2017/8/27: patch getOperatorResponse			by: daniel_hk
 * 2017/9/2: another way to restart the service		by: daniel_hk
 * 2017/9/2: patch convertRilCellInfoListToHal for MTK	by: daniel_hk
 * 2017/9/3: fix convertRilSignalStrengthToHal		by: daniel_hk
 * 2017/9/9: enhance convertRilSignalStrengthToHal	by: daniel_hk
 * 2017/9/9: fix setupDataCallResponse			by: daniel_hk
 * 2017/9/10: write my own my_enqueue			by: daniel_hk
 * 2017/9/16: fix getModemActivityInfoResponse		by: daniel_hk
 * 2017/9/16: patch setInitialAttachApn			by: daniel_hk
...
struct RadioImpl : public ::android::hardware::radio::V1_0::IRadio {
    int32_t mSlotId;
...
bool dispatchVoid(int serial, int slotId, int request) {
    ...
    android::my_enqueue(request, NULL, 0, android::FMT_VOID, pRI);
    return true;
}
/*
bool dispatchlocalVoid(int serial, int slotId, int request) {
    RequestInfo *pRI = android::addRequestToList(serial, slotId, request);
    if (pRI == NULL) {
	return false;
    }
    android::my_enqueue(request, NULL, 0, pRI);
    return true;
}
*/
bool dispatchString(int serial, int slotId, int request, const char * str) {
    ...
    bool ignoreBF = (pRI->pCI->proxyId != RIL_CMD_PROXY_1) &&
		(pRI->pCI->proxyId != RIL_CMD_PROXY_3);
    if (ignoreBF) {
	android::my_enqueue(request, pString, sizeof(char *), android::FMT_IGNORE, pRI);
	memsetAndFreeStrings(1, pString);
    }
    else {
	android::my_enqueue(request, pString, sizeof(char *), android::FMT_STR, pRI);
    }
...
bool dispatchStrings(int serial, int slotId, int request, bool allowEmpty, int countStrings, ...) {
    ...
    bool ignoreBF = (pRI->pCI->proxyId != RIL_CMD_PROXY_1) &&
		(pRI->pCI->proxyId != RIL_CMD_PROXY_3);
    if (ignoreBF) {
	android::my_enqueue(request, pStrings, countStrings * sizeof(char *),
				android::FMT_IGNORE, pRI);
	if (pStrings != NULL) {
	    for (int i = 0 ; i < countStrings ; i++) {
...
bool dispatchInts(int serial, int slotId, int request, int countInts, ...) {
    ...
    bool ignoreBF = (pRI->pCI->proxyId != RIL_CMD_PROXY_1) &&
		(pRI->pCI->proxyId != RIL_CMD_PROXY_3);
    if (ignoreBF) {
	android::my_enqueue(request, pInts, countInts * sizeof(int), android::FMT_IGNORE, pRI);
	if (pInts != NULL) {
#ifdef MEMSET_FREED
	...
	android::my_enqueue(request, pInts, countInts * sizeof(int), android::FMT_INTS, pRI);
    }
    return true;
}
...
bool dispatchCallForwardStatus(int serial, int slotId, int request,
    ...
    bool ignoreBF = (pRI->pCI->proxyId != RIL_CMD_PROXY_1) &&
		(pRI->pCI->proxyId != RIL_CMD_PROXY_3);
    if (ignoreBF) {
	android::my_enqueue(request, &cf, sizeof(cf), android::FMT_IGNORE, pRI);
	memsetAndFreeStrings(1, cf.number);
    }
    else {
	android::my_enqueue(request, &cf, sizeof(cf), android::FMT_CallFWST, pRI);
    }
...
bool dispatchRaw(int serial, int slotId, int request, const hidl_vec<uint8_t>& rawBytes) {
    ...
    android::my_enqueue(request, (void *) uData, rawBytes.size(), android::FMT_IGNORE, pRI);
...
bool dispatchIccApdu(int serial, int slotId, int request, const SimApdu& message) {
    ...
    android::my_enqueue(request, &apdu, sizeof(apdu), android::FMT_IccAPDU, pRI);
...
Return<void> RadioImpl::dial(int32_t serial, const Dial& dialInfo) {
    ...
    android::my_enqueue(RIL_REQUEST_DIAL, &dial, sizeOfDial, android::FMT_IGNORE, pRI);
...
Return<void> RadioImpl::setRadioPower(int32_t serial, bool on) {
    RLOGD("setRadioPower: serial %d on=%s", serial, (on)?"true":"false");
/* how to check whether in Airplane mode??
    if (on) {
	property_set("gsm.ril.eboot", "0");
...
Return<void> RadioImpl::setupDataCall(int32_t serial, RadioTechnology radioTechnology,
	...
	while ((i<4) && (dataCallCids[i] != -1))
	    i++;
	if (i < 4)
	    i++;	// = i+1 if found, otherwise use 0
	else i=0;
#if VDBG
    RLOGD("setupDataCall: radioTech:%u,profile=%u,apn:%s,user:%s,pw:%s,auth:%u,protocol:%s,i=%d",
	radioTechnology,dataProfileInfo.profileId,dataProfileInfo.apn.c_str(),
	dataProfileInfo.user.c_str(),dataProfileInfo.password.c_str(),
	dataProfileInfo.authType,protocol.c_str(),i);
#endif
	dispatchStrings(serial, mSlotId, RIL_REQUEST_SETUP_DATA_CALL, true, 8,
...
Return<void> RadioImpl::iccIOForApp(int32_t serial, const IccIo& iccIo) {
    ...
    android::my_enqueue(RIL_REQUEST_SIM_IO, &rilIccIo, sizeof(rilIccIo), android::FMT_SIMIO, pRI);
...
Return<void> RadioImpl::writeSmsToSim(int32_t serial, const SmsWriteArgs& smsWriteArgs) {
    ...
    android::my_enqueue(RIL_REQUEST_WRITE_SMS_TO_SIM, &args, sizeof(args),
...
Return<void> RadioImpl::sendCdmaSms(int32_t serial, const CdmaSmsMessage& sms) {
    ...
    // android::FMT_RAW needed alloc and removed
    android::my_enqueue(pRI->pCI->requestNumber, &rcsm, sizeof(rcsm), android::FMT_RAW, pRI);
    return Void();
}
...
Return<void> RadioImpl::acknowledgeLastIncomingCdmaSms(int32_t serial, const CdmaSmsAck& smsAck) {
    ...
    // android::FMT_RAW needed alloc and removed
    android::my_enqueue(pRI->pCI->requestNumber, &rcsa, sizeof(rcsa), android::FMT_RAW, pRI);
...
Return<void> RadioImpl::setGsmBroadcastConfig(int32_t serial,
    ...
    android::my_enqueue(pRI->pCI->requestNumber, gsmBciPtrs,
		num * sizeof(RIL_GSM_BroadcastSmsConfigInfo *), android::FMT_RAW, pRI);
...
Return<void> RadioImpl::setCdmaBroadcastConfig(int32_t serial,
    ...
    android::my_enqueue(pRI->pCI->requestNumber, cdmaBciPtrs,
		num * sizeof(RIL_CDMA_BroadcastSmsConfigInfo *), android::FMT_RAW, pRI);
...
Return<void> RadioImpl::writeSmsToRuim(int32_t serial, const CdmaSmsWriteArgs& cdmaSms) {
    ...
    android::my_enqueue(pRI->pCI->requestNumber, &rcsw, sizeof(rcsw), android::FMT_RAW, pRI);
    return Void();
}
...
//		iaa.authtype = -1;
	...
	char prop[32];
	char *p = prop;
	if (property_get("gsm.sim.operator.numeric", p, "")) {
	    RLOGI("gsm.sim.operator.numeric:%s", prop);
	    p = strchr(prop, ',');
	    if (p)
		*p++ = 0;
	    if (mSlotId == 0)
	...
	iaa.operatorNumeric = (char *) calloc(8, sizeof(char));
...
*/
	...
	android::my_enqueue(RIL_REQUEST_SET_INITIAL_ATTACH_APN, &iaa, sizeof(iaa),
				android::FMT_IGNORE, pRI);
...
bool dispatchImsGsmSms(const ImsSmsMessage& message, RequestInfo *pRI) {
    ...
    android::my_enqueue(pRI->pCI->requestNumber, &rism, sizeof(RIL_RadioTechnologyFamily) +
	    sizeof(uint8_t) + sizeof(int32_t) + dataLen, android::FMT_ImsSms, pRI);
/*
    for (int i = 0 ; i < countStrings ; i++) {
	memsetAndFreeStrings(1, pStrings[i]);
    }
...
bool dispatchImsCdmaSms(const ImsSmsMessage& message, RequestInfo *pRI) {
    ...
    // android::FMT_RAW
    android::my_enqueue(pRI->pCI->requestNumber, &rism, sizeof(RIL_RadioTechnologyFamily) +
	    sizeof(uint8_t) + sizeof(int32_t) + sizeof(rcsm), android::FMT_RAW, pRI);
...
/*    if (s_vendorFunctions->version < 15) {
	...
	android::my_enqueue(pRI->pCI->requestNumber, &params, sizeof(params), NULL, pRI);
...
Return<void> RadioImpl::nvReadItem(int32_t serial, NvItem itemId) {
    ...
    android::my_enqueue(pRI->pCI->requestNumber, &nvri, sizeof(nvri), android::FMT_IGNORE, pRI);
    return Void();
}
...
Return<void> RadioImpl::nvWriteItem(int32_t serial, const NvWriteItem& item) {
    ...
    android::my_enqueue(pRI->pCI->requestNumber, &nvwi, sizeof(nvwi), android::FMT_IGNORE, pRI);
...
Return<void> RadioImpl::setUiccSubscription(int32_t serial, const SelectUiccSub& uiccSub) {
    ...
    android::my_enqueue(pRI->pCI->requestNumber, &rilUiccSub, sizeof(rilUiccSub),
			android::FMT_RAW, pRI);
...
Return<void> RadioImpl::requestIccSimAuthentication(int32_t serial, int32_t authContext,
    ...
    android::my_enqueue(pRI->pCI->requestNumber, &pf, sizeof(pf), android::FMT_SIMAUTH, pRI);
...
Return<void> RadioImpl::setDataProfile(int32_t serial, const hidl_vec<DataProfileInfo>& profiles,
	...
	android::my_enqueue(RIL_REQUEST_SET_DATA_PROFILE, dataProfilePtrs,
			num * sizeof(RIL_DataProfileInfo *), android::FMT_DATAPROF, pRI);
/*
	freeSetDataProfileData(num, dataProfiles, dataProfilePtrs, 4,
		&RIL_DataProfileInfo::apn, &RIL_DataProfileInfo::protocol,
		&RIL_DataProfileInfo::user, &RIL_DataProfileInfo::password);*/
	...
	android::my_enqueue(RIL_REQUEST_SET_DATA_PROFILE, dataProfilePtrs,
		num * sizeof(RIL_DataProfileInfo_v15 *), android::FMT_IGNORE, pRI);
...
Return<void> RadioImpl::setAllowedCarriers(int32_t serial, bool allAllowed,
    ...
    android::my_enqueue(pRI->pCI->requestNumber, &cr, sizeof(RIL_CarrierRestrictions),
			android::FMT_IGNORE, pRI);
...
int radio::getSignalStrengthResponse(int slotId,
	...
	if (response == NULL || responseLen < sizeof(RIL_SignalStrength_v8)) {
	    RLOGE("getSignalStrengthResponse: Invalid response,responseLen=%d", (int)responseLen);
	    if (e == RIL_E_SUCCESS) responseInfo.error = RadioError::INVALID_RESPONSE;
	} else {
...
int convertResponseStringEntryToInt(char **response, int index, int numStrings) {
	...
	return (int)strtol(response[index], NULL, 16); 
	//** should be hex! not atoi(response[index]);
    }
...
int radio::getVoiceRegistrationStateResponse(int slotId,
	...
	if (response == NULL) {
	    ...
	    if (/*(numStrings < 14) &&*/ (numStrings < 4)) {
		RLOGE("getVoiceRegistrationStateResponse Invalid response: num=%d",numStrings);
#if VDBG
		char **resp = (char **) response;
		for (int i=0; i < numStrings; i++)
		    RLOGW("***response[0]:%s",resp[i]);
#endif
		if (e == RIL_E_SUCCESS) responseInfo.error = RadioError::INVALID_RESPONSE;
	    } else {
		char **resp = (char **) response;
		...
		voiceRegResponse.cssSupported = (numStrings > 4) ?
			ATOI_NULL_HANDLED_DEF(resp[7], 0) : 0;
		voiceRegResponse.roamingIndicator = (numStrings > 4) ?
			ATOI_NULL_HANDLED(resp[10]) : -1;
		voiceRegResponse.systemIsInPrl = (numStrings > 4) ?
			ATOI_NULL_HANDLED_DEF(resp[11], 0) : 0;
		voiceRegResponse.defaultRoamingIndicator = (numStrings > 4) ?
			ATOI_NULL_HANDLED_DEF(resp[12], 0) : 0;
...
const char *getname(int plmn) {
    for (int i=0; i < (int)NUM_ELEMS(spn); i++) {
	if (spn[i].plmn == plmn)
	    return spn[i].name;
    }
    return NULL;
...
int radio::getCellInfoListResponse(int slotId,
	...
	if (response == NULL || ((responseLen % sizeof(RIL_CellInfo_v12) != 0)/* &&
				 (responseLen % sizeof(RIL_CellInfo) != 0)*/)) {
	    RLOGE("getCellInfoListResponse: Invalid response,len=%d",(int)responseLen);
	    if (e == RIL_E_SUCCESS) responseInfo.error = RadioError::INVALID_RESPONSE;
	} else {
...
int radio::iccOpenLogicalChannelResponse(int slotId,
	...
	int numInts = responseLen / sizeof(int);
	if (response == NULL || responseLen % sizeof(int) != 0) {
	    RLOGE("iccOpenLogicalChannelResponse Invalid response: responseLen=%d", (int)responseLen);
	    if (e == RIL_E_SUCCESS) responseInfo.error = RadioError::INVALID_RESPONSE;
	} else {
	    int *pInt = (int *) response;
	    channelId = pInt[0];
...
int radio::getHardwareConfigResponse(int slotId,
	...
	if (response == NULL || responseLen % sizeof(RIL_HardwareConfig) != 0) {
	    RLOGE("getHardwareConfigResponse: invalid response=%d", (int)responseLen);
	    if (e == RIL_E_SUCCESS) responseInfo.error = RadioError::INVALID_RESPONSE;
	} else {
...
int radio::getModemActivityInfoResponse(int slotId,
	...
	// mtk not treated as error
	    ...
	    info.sleepModeTimeMs = 0;
	    info.idleModeTimeMs = 0;
...
int radio::radioStateChangedInd(int slotId,
	...
	RadioState radioState = (RadioState) s_vendorFunctions->onStateRequest((RIL_SOCKET_ID)slotId);
...
//    uint8_t *bytes = (uint8_t *)calloc(responseLen/2, sizeof(uint8_t));
    uint8_t *bytes = (uint8_t *)calloc(responseLen, sizeof(uint8_t));
    if (bytes == NULL) {
	...
	bytes[(responseLen+i)/2] = ((hexChar1 << 4) | hexChar2);	// @daniel, repeat again
    }
...
int radio::newSmsInd(int slotId, int indicationType,
	...
	pdu.setToExternal(bytes, pdu_len/2);
#if VDBG
	RLOGD("newSmsInd, pdu_len=%u, %s",pdu_len, (char*)response);
#endif
...
int radio::newSmsStatusReportInd(int slotId,
	...
	pdu.setToExternal(bytes, pdu_len/2);
#if VDBG
	RLOGD("newSmsStatusReportInd, pdu_len=%u, %s",pdu_len, (char*)response);
#endif
...
// *** seem ours is v6, must change types.hal in interface accordingly.
//	for the time being, skip last LTE and WCDMA as the old hal
...
int radio::currentSignalStrengthInd(int slotId,
				    ...
				    void *response, size_t responseLen) {
    if (radioService[slotId] != NULL && radioService[slotId]->mRadioIndication != NULL) {
	if (response == NULL || responseLen < sizeof(RIL_SignalStrength_v8)) { // can be v6=64, v8=52 & v10=56
	    RLOGE("currentSignalStrengthInd: invalid response = %d", (int)responseLen);
	    return 0;
	}
...
int radio::hardwareConfigChangedInd(int slotId,
    ...
    if (radioService[slotId] != NULL && radioService[slotId]->mRadioIndication != NULL) {
	if (response == NULL || responseLen % sizeof(RIL_HardwareConfig) != 0) {
	    RLOGE("hardwareConfigChangedInd: invalid response, responseLen=%d", (int)responseLen);
	    return 0;
	}
...
// *** handle MTK USOL commands
// RIL_UNSOL_RESPONSE_REGISTRATION_SUSPENDED
int radio::registrationSuspendInd(int slotId,
	...
	RLOGE("registrationSuspendInd: radioService[%d]->mRadioIndication == NULL", slotId);
    }
...
int radio::incomingCallInd(int slotId,
			   int indicationType, int token, RIL_Errno e, void *response,
			   size_t responseLen) {
    if (radioService[slotId] != NULL && radioService[slotId]->mRadioIndication != NULL) {
#if VDBG
	RLOGD("incomingCallInd");
#endif
	char **strings = (char **) response;
	int p0,p1,p2;
	...
	p1 = atoi(strings[0]);
	p2 = atoi(strings[4]);
	dispatchInts(-1, slotId, RIL_REQUEST_SET_CALL_INDICATION, 3, p0, p1, p2);

	Return<void> retStatus = radioService[slotId]->mRadioIndication->callStateChanged(
		convertIntToRadioIndicationType(indicationType));
	radioService[slotId]->checkReturnStatus(retStatus);
    } else {
	RLOGE("incomingCallInd: radioService[%d]->mRadioIndication == NULL", slotId);
    }
...
int radio::psNetworkStateChangedInd(int slotId,
				    int indicationType, int token, RIL_Errno e, void *response,
				    size_t responseLen) {
    if (radioService[slotId] != NULL && radioService[slotId]->mRadioIndication != NULL) {
#if VDBG
...
bool radio::isServiceConnected(int slotId)
{
    return (radioService[slotId] != NULL && radioService[slotId]->mRadioIndication != NULL);
}
