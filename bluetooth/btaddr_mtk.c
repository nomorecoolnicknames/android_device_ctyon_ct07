 ...
 *
 * The MTK vendor library (libbluetooth_mtk, loaded by libbt-vendor inside
 * the Bluetooth HAL process) programs the controller from the NVRAM
 * record /data/nvram/APCFG/APRDEB/BT_Addr (first 6 bytes of
 * ap_nvram_btradio_struct). Pie's android.hardware.bluetooth@1.0-impl needs
 * an address before it loads that library: it reads ro.bt.bdaddr_path,
 * ro.boot.btmacaddr or persist.service.bdroid.bdaddr and aborts when all
 * three are empty.
 *
 * This helper runs once NVRAM is ready and writes the factory address in
 * ASCII form to /data/misc/bluetooth/bdaddr (ro.bt.bdaddr_path). If NVRAM
 * holds no per-unit address (all zero, all 0xFF or the MTK default), a
 * stable locally administered address is derived from ro.serialno and
 * written back to NVRAM so the controller uses the same one.
 */
...
int main(void)
{
    ...
    /* The HAL runs as bluetooth:bluetooth. */
    if (fchown(fd, AID_BLUETOOTH, AID_BLUETOOTH) != 0)
        ALOGW("fchown %s: %s", OUT_PATH, strerror(errno));
    close(fd);
